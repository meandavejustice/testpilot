...
export default class NewsUpdatesDialog extends React.Component {
  ...
  render() {
    const { isExperimentEnabled, newsUpdates } = this.props;
    const { currentStep } = this.state;

    const atStart = (currentStep === 0);
    const atEnd   = (currentStep === newsUpdates.length - 1);

    const headerTitle = newsUpdates.map((u, idx) => (idx === currentStep) && (
        <h3 className={cn('modal-header lighter', {
          enabled: isExperimentEnabled({ addon_id: `@${u.experimentSlug}` })
        })}>{u.experimentSlug.split('-').join(' ')}</h3>
    ));

    const update = newsUpdates.map((u, idx) => (idx === currentStep) && (
        <div key={idx} className='tour-content'>
          <div className='tour-image'><img src={u.image} /></div>
          {u.content &&
